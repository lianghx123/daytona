...
[Volumes](/docs/en/volumes) provide persistent storage that can be shared across sandboxes. A volume can use Daytona-managed S3-compatible storage or reference an externally managed, pre-formatted JuiceFS filesystem. Daytona manages volume identity, access control, configuration, and mount orchestration; JuiceFS remains responsible for its metadata and data lifecycle. Multiple sandboxes can mount the same volume simultaneously, allowing data to be shared across sandboxes and persist independently of the sandbox lifecycle.
