...
`metaUrl` must be an absolute URL with a scheme. Do not embed a password in the URL; pass it through `credential.metaPassword`. The password is encrypted by the Daytona API and is never returned by volume or job APIs. `cacheSizeMiB` defaults to `10240` and controls the disposable local Runner cache.
