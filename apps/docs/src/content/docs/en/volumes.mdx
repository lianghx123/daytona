...
Volumes are FUSE-based mounts that provide shared file access across Daytona sandboxes. They enable sandboxes to read from large files instantly - no need to upload files manually to each sandbox. By default, Daytona manages the volume in its configured S3-compatible object store. You can also register an externally managed, pre-formatted JuiceFS filesystem.
...
### Use an external JuiceFS filesystem

Format and operate the JuiceFS filesystem outside Daytona, then register its metadata URL when creating the volume. Daytona validates the configuration and manages access and mounting, but it does not format or destroy the JuiceFS filesystem. Deleting the Daytona volume removes only the Daytona reference, so the external data remains available.

`metaUrl` must be an absolute URL with a scheme. Do not embed a password in the URL; pass it through `credential.metaPassword`. The password is encrypted by the Daytona API and is never returned by volume or job APIs. `cacheSizeMiB` defaults to `10240` and controls the disposable local Runner cache. `capacityGiB` defaults to `50`; before mounting, the Runner applies it with `juicefs config --capacity` when the filesystem setting differs. JuiceFS capacity is filesystem-wide, so every client and every Daytona Volume reference using the same metadata URL observes the same enforced capacity.

Use the optional `bucket` URL when the object storage location recorded in JuiceFS metadata is not reachable from your Runners. Daytona passes this value to `juicefs mount --bucket`; when omitted, JuiceFS uses the bucket stored in its metadata. Bucket URLs may contain a path and query parameters, but must include a scheme and hostname and must not contain embedded credentials.

```typescript
const volume = await daytona.volume.create('shared-juicefs', {
  type: 'juicefs',
  metaUrl: 'redis://metadata.internal:6379/1',
  bucket: 'https://minio.runner-network.internal:9000/juicefs-data',
  cacheSizeMiB: 20480,
  capacityGiB: 100,
  credential: { metaPassword: process.env.JUICEFS_META_PASSWORD },
})
```

```python
from daytona_api_client import CreateVolumeBackendDto, JuiceFSVolumeCredentialDto, VolumeBackendType

volume = daytona.volume.create(
    "shared-juicefs",
    CreateVolumeBackendDto(
        type=VolumeBackendType.JUICEFS,
        meta_url="redis://metadata.internal:6379/1",
        bucket="https://minio.runner-network.internal:9000/juicefs-data",
        capacity_gib=100,
        cache_size_mi_b=20480,
        credential=JuiceFSVolumeCredentialDto(meta_password="secret"),
    ),
)
```

When a JuiceFS volume is created, the API performs a short DNS/TCP connectivity check for the metadata endpoint and for an explicitly configured bucket. `READY` means the API control plane could reach those endpoints; it does not guarantee reachability from every Runner region. Before mounting, each Runner checks metadata connectivity, runs `juicefs status`, checks the effective bucket endpoint, and then performs the mount. Regional failures identify the Runner, region, failure stage, and sanitized `hostname:port` without exposing passwords, URL paths, or query parameters.

The backend configuration is immutable. To change it, delete the Daytona volume reference and create a new one. Daytona currently supports `managed_s3` and `juicefs`; external S3 backends are not supported.
...
await producer.fs.uploadFile(
  Buffer.from('shared payload'),
...
await aliceSandbox.fs.uploadFile(
  Buffer.from("alice's data"),
  `${mountDir}/notes.txt`
)
...
await bobSandbox.fs.uploadFile(
  Buffer.from("bob's data"),
  `${mountDir}/notes.txt`
)
```
